                                    ...
                                    )
{
  cout << "       ---> Tu moge przetwarzyc element: " << ElemName << endl;
	...
	XMLSize_t i = 0;
	if (ElemName == "Robot") {
		pom = xercesc::XMLString::transcode(Attrs.getValue(i));
		s = string(pom);
		_pScn->x = std::stof(pom);
		i = 1;
		pom = xercesc::XMLString::transcode(Attrs.getValue(i));
		s = string(pom);
		_pScn->y = std::stof(pom);
		i = 2;
		pom = xercesc::XMLString::transcode(Attrs.getValue(i));
		s = string(pom);
		_pScn->kat = std::stof(pom);
	}
	else if (ElemName == "Object") {
		i = 1;
		double pomx, pomy;
		pom = xercesc::XMLString::transcode(Attrs.getValue(i));
		s = string(pom);
		...
		s = string(pom);
		pomy = std::stof(pom);
		_pScn->obiekty.push_back(Punkt(pomx,pomy));
   	ofstream NowyPlik;
		char nazwa[20] = "punkt";
		char lancuch[15];
		sprintf(lancuch,"%d",_pScn->obiekty.size());
		strcat(nazwa,lancuch);
   	NowyPlik.open(nazwa);
   	NowyPlik << pomx << " " << pomy << endl;
   	NowyPlik.close();
	}
}
