/*
$HeadURL: svn+ssh://dperdek@panamint.ict.pwr.wroc.pl/home/dperdek/repos/z1/trunk/inc/lacze_do_gnuplota.hh $
$Author: dperdek $
...
namespace PzG {
   ...
   */
  enum RodzajRysowania { RR_Ciagly /*! Linia ma być rysowana jako sekwencja punktów */, 
  ...
  class InfoPlikuDoRysowania {
    public:
     /*!
      * \brief Inicjalizuje obiekt niezbędnymi danymi.
      *
      * Inicjalizuje obiekt.
...
class LaczeDoGNUPlota {
 protected:
  ...
  static std::list<InfoPlikuDoRysowania>  _InfoPlikow;
   ...
   /*!
    * \brief Deskryptor wejścia standardowego GNUPlota
    *
  ...
  int           _Wejscie_GNUPlota;
   /*!
    * \brief Deskryptor wyjścia standardowego GNUPlota
    *
  ...
  float  _Zskala;
    ...
    *
    *  Wartość tego pola definiuje rotację rysunku (zmianę punktu patrzenia)
    *  względem osi \e OX.
    */
  float  _Xrotacja;
   /*!
 ...
 public:
  ...
  /*!
   * \brief Wymusza odrysowanie okna GNUPlota
   *
  ...
  bool Rysuj();
  /*!
   * \brief Wymusza zapis rysunku do pliku
   *
  ...
  /*!
   * \brief Inicjalizuje podstawowe struktury danych 
   *
   * Inicjalizuje podstawowe struktury danych. Nie uruchamia jednak GNUPlota
   * i nie tworzy do niego łącza. 
   */
  LaczeDoGNUPlota();
  /*!
   * \brief Zamyka łącze do GNUPlota
   *
   *  Wymusza zakończenie pracy GNUPlota i zamyka wcześniej utworzone łącze.
   */
