...
/*!
 *\file
 *\brief	Plik zawierający implementację wybranych metod z nagłówka lacze_do_gnuplota.
*/
...
namespace PzG {
 ...
 void LaczeDoGNUPlota::UstawTytulOkna(const char* Tytul)
  {
    ostringstream Polecenie_UstawieniaTytuluOkna;
    Polecenie_UstawieniaTytuluOkna << "set term X11 title \""
				   << Tytul << "\"";
    PrzeslijDoGNUPlota(Polecenie_UstawieniaTytuluOkna.str().c_str());
  }
 ...
 bool LaczeDoGNUPlota::UtworzProcesPotomny()
 {
  ...
  switch (fork()) {
    ...
    case  0: 
       ...
       execlp(Comm = "gnuplot","gnuplot","-title","Oscyloskop nr 1",NULL);
